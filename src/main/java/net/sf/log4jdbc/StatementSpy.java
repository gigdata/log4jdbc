...
public class StatementSpy implements Statement, Spy {
	...
	public StatementSpy(ConnectionSpy connectionSpy, Statement realStatement) {
		if (realStatement == null) {
			throw new IllegalArgumentException(
					"Must pass in a non null real Statement");
		}
		if (connectionSpy == null) {
			throw new IllegalArgumentException(
					"Must pass in a non null ConnectionSpy");
		}
	 ...
	 * @param execTime
	 *            amount of time that the jdbc driver was chugging on the SQL
	 *            before it threw an exception.
	 */
	...
	protected void reportStatementSql(String sql, String methodCall) {
		// redirect to one more method call ONLY so that stack trace search is
		// consistent
	 ...
	 *            the name of the method that was running the SQL
	 */
	protected void reportStatementSqlTiming(long execTime, String sql,
			String methodCall) {
		// redirect to one more method call ONLY so that stack trace search is
		// consistent
		// with the reportReturn calls
		_reportSqlTiming(
				execTime,
				(DriverSpy.StatementUsageWarn ? StatementSqlWarning : "") + sql,
				methodCall);
	}
	...
	public int executeUpdate(String sql, String[] columnNames)
		...
		try {
			...
			reportStatementSqlTiming(System.currentTimeMillis() - tstart, sql,
					methodCall);
			return reportReturn(methodCall, result);
		} catch (SQLException s) {
			reportException(methodCall, s, sql, System.currentTimeMillis()
					- tstart);
			throw s;
		}
	}
	...
	public int[] executeBatch() throws SQLException {
		...
		StringBuffer batchReport = new StringBuffer("batching " + j
				+ " statements:");
		...
		try {
			...
			reportSqlTiming(System.currentTimeMillis() - tstart, sql,
					methodCall);
		} catch (SQLException s) {
			reportException(methodCall, s, sql, System.currentTimeMillis()
					- tstart);
	...
	public ResultSet getGeneratedKeys() throws SQLException {
		...
		try {
			...
			if (r == null) {
				...
				return (ResultSet) reportReturn(methodCall, new ResultSetSpy(
						this, r));
			}
		} catch (SQLException s) {
	...
	public boolean getMoreResults(int current) throws SQLException {
		...
		try {
			return reportReturn(methodCall,
					realStatement.getMoreResults(current));
		} catch (SQLException s) {
			reportException(methodCall, s);
			throw s;
		}
	...
	public int getResultSetConcurrency() throws SQLException {
		...
		try {
			return reportReturn(methodCall,
					realStatement.getResultSetConcurrency());
		} catch (SQLException s) {
			reportException(methodCall, s);
			throw s;
		}
